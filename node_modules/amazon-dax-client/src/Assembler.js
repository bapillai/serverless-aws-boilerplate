...
class Assembler {
  ...
  _assembleError() {
    ...
    let excInfo = this.dec.decodeObject();

    let requestId = excInfo ? excInfo[0] : null;
    let errorCode = excInfo ? excInfo[1]: null;
    let statusCode = excInfo ? excInfo[2] : -1;
    let retryable;

    return new DaxServiceError(errMsg, errorCode, retryable, requestId, statusCode, codeSeq);
  }
